...
class Requests extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('user_id, doc_id, comment, state_time', 'required'),
			array('user_id', 'numerical', 'integerOnly'=>true),
			array('doc_id', 'length', 'max'=>20),
			array('state', 'length', 'max'=>4),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, user_id, doc_id, comment, state, state_time', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('state_time',$this->state_time,true);
